...
rm(list=ls())
results_tbl <- read.csv('./Experiment_4/Exp_4_ADP_vr/ResultsData/results_log_bias.csv', header = TRUE, sep = ',', stringsAsFactors = TRUE)
...
figures_folder = "./Experiment_4/Exp_4_ADP_vr/figura_individuales"
...
#1       1 No Visual Info            9             1.14
#2       6 No Visual Info            3.65          1.09
#3       6 No Visual Info            4.9           1.07
#4       6 No Visual Info            6.65          1.22
#5       6 No Visual Info            9             1.10

idx = results_tbl$subject == "1" & results_tbl$room_condition == 'No Visual Info' & 
  results_tbl$target_distance == 9  
results_tbl = results_tbl[!idx,]

idx = results_tbl$subject == "6" & results_tbl$room_condition == 'No Visual Info' & 
...
tabla.outlier <- results_tbl %>% 
  filter(room_condition == 'Larger VE') %>% 
  group_by(subject, room_condition, target_distance) %>%
  summarise(mBiasUnsigned  = mean(log_bias_unsigned_m,na.rm=TRUE))  %>%
  ungroup()
res3 <- outliers_mad(x = tabla.outlier$mBiasUnsigned,threshold = 3 ,na.rm=TRUE)
...
#1       1 Larger VE                  9           0.795
#2       2 Larger VE                  2           0.824
#3       2 Larger VE                  2.7         0.880

idx = results_tbl$subject == "1" & results_tbl$room_condition == 'Larger VE' & 
  results_tbl$target_distance == 9  
results_tbl = results_tbl[!idx,]

idx = results_tbl$subject == "2" & results_tbl$room_condition == 'Larger VE' & 
...
write_csv(x =results_tbl ,file ="./Experiment_4/Exp_4_ADP_vr/ResultsData/Dresults_nuevos.csv")
