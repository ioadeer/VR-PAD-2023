...
results_tbl <- read.csv("Experiment_4/Exp_4_ADP_vr/ResultsData/DresultsExp4.csv", header = TRUE, sep = ',', stringsAsFactors = TRUE)
#por que los pone por orden alfabetico
results_tbl <- results_tbl %>%
  mutate(
    room_condition = case_when(
      room_condition == "No visual information" ~ "NVI",
      room_condition == "Larger VE" ~ "VE",
    )
  )

#results_tbl <- factor(results_tbl$room_condition, levels=c("No visual information", "Larger VE"))
