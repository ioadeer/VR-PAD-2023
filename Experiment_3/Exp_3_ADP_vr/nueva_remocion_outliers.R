...
rm(list=ls())
results_tbl <- read.csv('./Experiment_3/Exp_3_ADP_vr/ResultsData/results_log_bias.csv', header = TRUE, sep = ',', stringsAsFactors = TRUE)
...
#figures_folder = "./Experiment_3/Exp_3_ADP_vr/figura_individuales"
...
idx = results_tbl$subject == "7" & results_tbl$room_condition == 'No Visual information' & 
...
tabla.outlier <- results_tbl %>% 
  filter(room_condition == 'Virtual Environment') %>% 
  group_by(subject, room_condition, target_distance) %>%
  summarise(mBiasUnsigned  = mean(log_bias_unsigned_m,na.rm=TRUE))  %>%
  ungroup()
res3 <- outliers_mad(x = tabla.outlier$mBiasUnsigned,threshold = 3 ,na.rm=TRUE)
...
idx = results_tbl$subject == "7" & results_tbl$room_condition == 'Virtual Environment' & 
  results_tbl$target_distance == 2  
results_tbl = results_tbl[!idx,]

idx = results_tbl$subject == "7" & results_tbl$room_condition == 'Virtual Environment' & 
  results_tbl$target_distance == 2.65  
results_tbl = results_tbl[!idx,]
...
write_csv(x =results_tbl ,file ="./Experiment_3/Exp_3_ADP_vr/ResultsData/Dresults_nuevos.csv")
