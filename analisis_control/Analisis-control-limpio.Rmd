...
# tabla.ind.collapsed <- tabla.ind %>%
#   group_by(condicion_sala, distancia) %>%
#   summarize(mean_sd = sd(respuesta[,"sd"]))

 tabla.ind.collapsed_sd <- tabla.ind %>%
   mutate(relative_sd = mean(respuesta[,"sd"]))
...
tabla.pob.collapsed_sd <- tabla.pob %>%
  mutate(relative_sd = respuestapob[,"sd"]/distancia) %>%
  group_by(condicion_sala)  %>%
