...
En esta sección vamos a ver la variabilidad. Por un lado tenemos 
...
SD colapsada porcentual 

```{r standard collapsed deviation, echo=FALSE, message=FALSE, warning=FALSE}

tabla.pob_collapsed <- tabla.pob %>%
  group_by(condicion_sala) %>%
  summarize(sd_mean_condicion = mean(respuestapob[,"sd"]),
            sem  = sd(respuestapob[,"sd"])/sqrt(length(respuestapob[,"sd"])))

tabla.collapsed_sd <- tabla.ind %>%
  group_by(nsub) %>%
  mutate(
    collapsed_percentual_sd = mean(respuesta[,"sd"]) * 100 
  )
```
