...
# Intra para los de primer bloque Coincidente
...
first_block_coincident <- results_tbl %>%
  subset(block == 1 & room_condition == "Coincident VE")

second_block_smaller <- results_tbl %>%
  subset(block == 2 & room_condition == "Smaller VE")

results_tbl <- rbind(first_block_coincident,second_block_smaller)  
...
eq1 <- substitute("1B Coincident VE:" ~~~ italic(y) == k %.% italic(X)^italic(a),
                  list(k = round(10^mDist1stats$tidy_data$estimate[[1]],digits = 2),
                       a = round(mDist1stats$tidy_data$estimate[[2]], digits = 2)))
eq2 <- substitute("2B Smaller VE:"~~~italic(y) == k %.% italic(X)^italic(a),
...
main_figure <- annotate_figure(main_figure, top = text_grob("First block coincident, second small", 
                                      color = "black", face = "bold", size = 12))

# save plot ---------------------------------------------------------------

figures_folder = "./new_Exp_1_ADP/figura_final/"
mi_nombre_de_archivo = paste(figures_folder, .Platform$file.sep, "1B_coincident_2B_small", ".png", sep = '')
