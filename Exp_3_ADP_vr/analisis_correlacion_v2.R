...
tabla.raw <- read.csv('./Exp_3_ADP_vr/data/oscuras_vr_sin_outliers.csv', header = TRUE, sep = ' ', stringsAsFactors = TRUE)
tabla.dimensions <- read.csv('./Exp_3_ADP_vr/data/tamanio_de_sala_sin_outliers_oscuras_virtual.csv', header = TRUE, sep = ' ', stringsAsFactors = TRUE)

tabla.dist_max <- tabla.raw %>%
  group_by(nsub, condicion_sala) %>%
  summarise(distancia_max = max(respuesta))

tabla.dist_max <- tabla.dist_max %>%
  filter(distancia_max > 0)

lookup <- c( Subject = "nsub")
tabla.dist_max <- rename(tabla.dist_max, all_of(lookup))

tabla.depth_oscuras = merge(x =tibble(tabla.dimensions[c('Subject', 'No_visual_info_depth')]), 
                            y = filter(tabla.dist_max, condicion_sala == 'OSCURAS'), 
                            by= "Subject")


tabla.depth_oscuras <- tabla.depth_oscuras %>%
  filter(!No_visual_info_depth >= 20)

tabla.depth_oscuras <- tabla.depth_oscuras %>%
   rename("perc_depth" = "No_visual_info_depth")

tabla.depth_visual = merge(x =tibble(tabla.dimensions[c('Subject', 'VE_depth')]), 
                           y = filter(tabla.dist_max, condicion_sala == 'VR'), 
                           by= "Subject")

tabla.depth_visual <- tabla.depth_visual %>%
  rename("perc_depth" = "VE_depth")
...
tabla.correlacion <- tabla.correlacion %>%
  mutate(log_perc_depth = log(perc_depth),
         log_distancia_max = log(distancia_max))

tabla.correlacion$condicion_sala = factor(tabla.correlacion$condicion_sala, levels=c("OSCURAS",
                                                                           "VR"))
...
correlation_plot <- ggplot(tabla.correlacion, 
  ...
  scale_color_manual(values = c("#E69F00", "#000000"), 
                     labels = c("No visual info", "Virtual environment"))+
