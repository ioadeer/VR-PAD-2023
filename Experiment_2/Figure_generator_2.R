...
f7 <- ggplot(tabla.pob_logSB, aes(x=target_distance, 
                                  y =(10^MlogSB-1)*100,
                                  #y =MlogSB, 
  ...
  geom_pointrange(aes(x = target_distance, 
                  ...
                  position = position_jitterdodge(jitter.width = .1,
                                                  jitter.height = 0,
                                                  dodge.width = .1 ))+
  geom_line(data = Final.FixedlogSB, aes(x = target_distance, y =(10^fit-1)*100, group=room_condition, color=room_condition))+
  #geom_line(data = Final.FixedlogSB, aes(x = target_distance, y =fit, group=room_condition, color=room_condition))+
  scale_x_continuous(name="Distance source (m)", limits = c(1,10)) +
  scale_y_continuous(name="Mean log signed bias (%)",   limits = c(-70,0)) +
...
violin_depth <- ggplot(dimensions.depth, aes(x=Condition, y=Depth,  fill=Condition)) +
  ...
  annotate("segment", x = 1.1, xend = 1.9, y = 18, yend = 18, colour = "black", linewidth=.5, alpha=1,)+
  labs(
...
mi_nombre_de_archivo = paste(figures_folder, .Platform$file.sep, "main_4", ".png", sep = '')
