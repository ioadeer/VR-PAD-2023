...
Final.Fixed<-effect(c("condicion_sala*log(distancia)", m.distancia, xlevels=list(log(distancia))))
...
tabla.oscuras.pob <- tabla.oscuras.pob %>%
  mutate(
    log_respuesta = log10(respuestapob[,"mean"]),
    log_distancia = log10(distancia)
  )

tabla.oscuras.ind <- tabla.oscuras.ind %>%
  mutate(
    log_respuesta = log10(respuesta[,"mean"]),
    log_distancia = log10(distancia)
  )
...
g1 <- ggplot() +
  ...
  #geom_point(data = tabla.oscuras.ind, mapping = aes(x=log_distancia, y=log10(respuesta[,"mean"])),
  ...
  geom_abline(intercept = 0, slope = 1, linetype="dashed") +
  annotate("text",x = 0.5, y = 10, label= eqn, parse = TRUE) +
  theme_linedraw(base_size = 9)

k = m.distancia.oscuras[1]$coefficients[[1]]
a = m.distancia.oscuras[1]$coefficients[[2]]
g2 <- ggplot() +
  geom_line(data= tabla.oscuras.pob, aes(x = distancia, y = 10^k * distancia^(a)))+
...
```{r log 10 power function visual, echo=FALSE, message=FALSE, warning=FALSE}
g2 <- ggplot() +
  geom_line(data= tabla.oscuras.pob, aes(x = distancia, y = 10^k * distancia^(a)))+
  geom_point(data= tabla.oscuras.pob, mapping = aes(x = distancia, y = 10^k * distancia^(a)), color= "red")+
  geom_point(data = tabla.oscuras.pob, mapping = aes(x=distancia, y=respuestapob[,"mean"]))+
  geom_point(data = tabla.oscuras.ind, mapping = aes(x=distancia, y=respuesta[,"mean"]), 
             color='blue', alpha=.8, shape=1, size=2.2, stroke=.2) + 
  scale_y_continuous(transform="identity", name="Perceived distance (m)",  breaks=c(0,2,2.7,3.65,4.9,6.65,9.0), labels=c(0,2,2.7,3.65,4.9,6.65,9.0), minor_breaks=TRUE) +
  scale_x_continuous(transform = "identity", name="Distance source (m)", breaks=c(0,2,2.7,3.65,4.9,6.65,9.0), labels=c(0,2,2.7,3.65,4.9,6.65,9.0), minor_breaks=TRUE) +
  geom_abline(intercept = 0, slope = 1, linetype="dashed") +
  ggtitle("Sala oscuras ajuste con funcion de potencia") +
  annotate("text",x = 5, y = 10, label= eqn, parse = TRUE) +
  theme_linedraw(base_size = 9)
...
g1 <- ggplot() +
  ...
  geom_point(data = tabla.visual.ind, mapping = aes(x=log_distancia, y=log(respuesta[,"mean"])), 
             color='blue', alpha=.8, shape=1, size=2.2, stroke=.2) + 
...
```{r log exp power function visual bacjup, echo=FALSE, message=FALSE, warning=FALSE}
tabla.visual.ind <- tabla.ind %>%
  filter(condicion_sala =="VISUAL")
